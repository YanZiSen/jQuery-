...
if(document.readyState==='complete'){
  setTimeout(fn);//IE hack写法
}
  document.addEventListener()
  window.addEventListener()  哪一个块哪一个加载 为的是有的浏览器有缓存

undefined和null没有对应的包装对象
  tostring.call() [object A*]比较靠谱
  class2type属性挂载
  type:function(obj){
    if(obj==null){
      return  String(obj);
...
IE9以下的浏览器中不能序列化标签link script 及用ｉｎｎｅｒＨＴＭＬ不能转化　解决方式是包一层ｄｉｖ　Ｐ５１页
